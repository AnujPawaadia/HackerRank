...
struct word get_word(char *text,int beg,int end){
    struct word answer;
    answer.data=calloc(end-beg+1, sizeof(char));
    int idex=0;
    for(int i=beg;i<=end;i++){
        answer.data[idex]=text[i];
        ++idex;
    }
    answer.data[idex]=0;
    return answer;
...
struct sentence get_sentence(char *text,int beg,int end){
    ...
    for( i=beg;i<=end;i++){
        if(text[i]==' '){
            answer.data[idex++]=get_word(text, start, i-1);
            start=i+1;
        }
    }
    answer.data[idex]=get_word(text, start, i-1);
...
struct paragraph get_paragraph(char *text,int beg,int end){
    ...
    for(i=beg;i<=end;i++){
        if(text[i]=='.'){
            answer.data[idex++]=get_sentence(text, start, i-1);
            start=i+1;
        }
    }
    return answer;
...
struct document get_document(char* text) {
    ...
    for(i=0;i<strlen(text);i++){
        if(text[i]=='\n'){
            answer.data[idex++]=get_paragraph(text, start, i-1);
            start=i+1;
        }
    }
    answer.data[idex]=get_paragraph(text, start, i-1);
